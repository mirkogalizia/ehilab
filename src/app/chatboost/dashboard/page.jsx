            ...
            <div className="flex gap-2">
              <Button onClick={()=>{
                if(newPhone){
                  ...
                  setNewPhone(''); setShowNewChat(false);
                }
              }} className="flex-1 bg-black text-white">Avvia</Button>
              <Button variant="outline" onClick={()=>setShowNewChat(false)} className="flex-1">Annulla</Button>
            </div>
          </div>
        )}
      </div>
            ...
            <div className="space-y-3">
              {filtered.map((msg,idx)=>(
                <div key={idx} className={`flex flex-col ${msg.from==='operator'?'items-end':'items-start'}`}>
                  <div className={`px-4 py-2 rounded-xl text-sm shadow-md max-w-[70%] ${
