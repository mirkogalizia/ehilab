...
export default function ChatPage() {
  ...
  useEffect(() => {
    if (!user?.uid) return;
    const q = query(collection(db, 'messages'), where('user_uid', '==', user.uid), orderBy('timestamp', 'asc'));
    ...
    return Object.entries(chatMap)
      .map(([phone, msgs]) => {
        ...
        return {
          ...
          lastMsgText: lastMsg.text || (lastMsg.type === 'image' ? '📷 Immagine' : '📎 File'),
          unread,
        };
      })
  ...
  useEffect(() => {
    ...
    if (!lastMsg) return setCanSendMessage(true);
  ...
  // Marca come letti i messaggi
  useEffect(() => {
    if (!selectedPhone || !user?.uid || allMessages.length === 0) return;
    const unreadMsgIds = allMessages.filter((m) => m.from === selectedPhone && m.read === false).map((m) => m.id);
  ...
  const sendMessage = async () => {
    if (!selectedPhone || (!messageText && !selectedMedia) || !userData) return;
    let mediaUrl = null;
    let msgType = 'text';

    if (selectedMedia) {
      const fileRef = ref(storage, `media/${user.uid}/${selectedPhone}/${Date.now()}_${selectedMedia.name}`);
      await uploadBytes(fileRef, selectedMedia);
      mediaUrl = await getDownloadURL(fileRef);
      msgType = selectedMedia.type.startsWith('image/') ? 'image' : 'document';
    }

    const payload =
      msgType === 'text'
        ? { messaging_product: 'whatsapp', to: selectedPhone, type: 'text', text: { body: messageText } }
        : msgType === 'image'
        ? { messaging_product: 'whatsapp', to: selectedPhone, type: 'image', image: { link: mediaUrl, caption: messageText } }
        : { messaging_product: 'whatsapp', to: selectedPhone, type: 'document', document: { link: mediaUrl, caption: messageText } };
        ...
        <div className="flex flex-col flex-1 bg-gray-100 relative">
          {!canSendMessage && (
            <div className="absolute top-0 left-0 right-0 bg-yellow-200 border border-yellow-400 text-yellow-900 text-center py-2 font-semibold z-10">
              ⚠️ La finestra di 24h è chiusa. Puoi inviare solo template WhatsApp.
            </div>
          )}
              ...
              {filtered.map((msg, idx) => (
                <div key={idx} className={`flex flex-col ${msg.from === 'operator' ? 'items-end' : 'items-start'}`}>
                  ...
                  >
                    {msg.mediaUrl ? (
                      msg.type === 'image' ? (
                        <img src={msg.mediaUrl} alt="img" className="rounded-lg max-h-48" />
                      ) : (
          ...
          <div className="flex items-center gap-2 p-3 bg-white border-t sticky bottom-0">
            ...
            <label className="cursor-pointer">
              <Camera size={22} className="text-gray-600 hover:text-black" />
              <input type="file" accept="image/*" className="hidden" onChange={(e) => setSelectedMedia(e.target.files[0])} />
            </label>
            {/* Allegati */}
            <label className="cursor-pointer">
              <Paperclip size={22} className="text-gray-600 hover:text-black" />
              <input type="file" className="hidden" onChange={(e) => setSelectedMedia(e.target.files[0])} />
            </label>
            <Input
            ...
            <Button
              onClick={sendMessage}
              disabled={(!messageText && !selectedMedia) || !canSendMessage}
