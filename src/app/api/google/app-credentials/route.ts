...
export async function POST(req: NextRequest) {
  try {
    ...
    await adminDB.doc(PATH(uid)).set({
      client_id,
      client_secret,
      redirect_uri: redirect_uri || `${process.env.NEXT_PUBLIC_BASE_URL}/api/google/oauth/callback`,
      updatedAt: new Date(),
    }, { merge: true });
...
export async function DELETE(req: NextRequest) {
  try {
    const uid = await getUidFromAuthHeader(req.headers.get('authorization'));
    await adminDB.doc(PATH(uid)).delete();
    return NextResponse.json({ ok: true });
  } catch (e: any) {
    return NextResponse.json({ error: e.message }, { status: 400 });
  }
